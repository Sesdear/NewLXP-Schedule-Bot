...
async def cmd_tomorrow(message: Message):
    ...
    if not token:
        await message.answer("❌ Вы не авторизованы в NewLXP.\nПожалуйста, войдите: /signin")
        return
    ...
    try:
        ...
        day_after_tomorrow_str = (today_local + timedelta(days=2)).date().isoformat()
        
        classes = get_schedule(
            token=token,
            date_from=tomorrow_str,
            date_to=day_after_tomorrow_str
        )
        ...
        logging.exception(f"Ошибка в /tomorrow")
